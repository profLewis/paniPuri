...
Main application (`panipuri.py`) with two playback engines, web player, iOS app, and supporting tools:
...
- **Web Player** (`player.html`) - Self-contained HTML with SVG pan layout, hover-to-play, synth/samples toggle, 16-voice polyphony. Synth uses additive synthesis with attack bloom, mallet noise, brightness EQ, and soft limiting
- **iOS App** (`PaniPuri-iOS/`) - Native SwiftUI app with multi-touch polyphonic playback, AVAudioEngine sample/synth modes, Canvas-rendered pan layout
...
- **Colab Notebook** (`panipuri_colab.ipynb`) - Browser-based synthesis demo, no install needed
...
panipuri_colab.ipynb     # Google Colab notebook for browser-based synthesis
...
PaniPuri-iOS/            # Native iOS app (SwiftUI + AVAudioEngine)
  PaniPuri/Models/       #   Data models (NoteShape, CalibrationData, NoteIdentity)
  PaniPuri/Views/        #   SwiftUI views (SteelPanView, ControlsView)
  PaniPuri/Touch/        #   Multi-touch handling (PanTouchView, PointInPolygon)
  PaniPuri/Audio/        #   Audio engine (AudioEngine, SamplePlayer, SynthPlayer)
  PaniPuri/Resources/    #   Bundled WAVs, calibration.json, note_shapes.json
